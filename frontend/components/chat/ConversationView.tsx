...
export function ConversationView() {
  ...
  const isGenerating = useConversationStore(s => !!s.currentAbortController);
  ...
  return (
    <div className="flex flex-col h-full w-full bg-background relative overflow-hidden">
      ...
      >
        {messages.length === 0 ? (
          <EmptyState onSelectPrompt={(prompt) => sendMessage(prompt)} isStreaming={isGenerating} />
        ) : (
          <div className="pb-8">
      ...
      {messages.length > 0 && (
        <div className="absolute bottom-0 left-0 right-0 p-4 pb-6 md:p-6 md:pb-8 bg-gradient-to-t from-background via-background/95 to-transparent z-10 pointer-events-none animate-in slide-in-from-bottom-4 duration-500">
          <div className="max-w-[1000px] mx-auto w-full relative pointer-events-auto">
            <Composer onSend={sendMessage} onCancel={cancelRequest} isStreaming={isGenerating} />
          </div>
        </div>
      )}
    </div>
