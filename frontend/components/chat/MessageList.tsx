...
export function MessageList({ messages }: MessageListProps) {
  ...
  return (
    <div className="flex flex-col space-y-8 py-8 w-full max-w-full md:max-w-4xl mx-auto px-2 md:px-4">
      {messages.map((msg) => (
        <div
          key={msg.id}
          className={cn(
        ...
        >
          {msg.role === "user" ? (
            <div className="flex flex-col max-w-[95%] md:max-w-[85%] bg-primary text-primary-foreground px-5 py-3.5 rounded-2xl rounded-tr-sm shadow-sm">
              {msg.attachments && msg.attachments.length > 0 && (
                <div className="flex flex-wrap gap-2 mb-2">
                  {msg.attachments.map((file: any) => (
                    <div
                      key={file.id}
                      className="flex items-center gap-2 bg-primary-foreground/10 border border-primary-foreground/20 rounded-xl px-3 py-1.5 text-sm"
                    >
            ...
            <div className="flex flex-col w-full space-y-4">
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center space-x-2 text-xs w-full">
                          ...
                          {step.status === "running" && (
                            <CircleDashed className="h-3 w-3 text-blue-500 animate-spin" />
                          )}
                          {step.status === "error" && (
              ...
              {/* Prose first */}
              <div className="pl-1 w-full overflow-x-auto custom-scrollbar">
                <AssistantAnswerText
                  text={msg.answerText}
