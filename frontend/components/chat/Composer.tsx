...
import { Send, Loader2, Paperclip, Mic, ArrowUpRight, Square, File as FileIcon, X, CheckCircle2, AlertCircle } from "lucide-react";
...
export function Composer({ onSend, onCancel, isStreaming, className = "" }: ComposerProps) {
  ...
  const stagedAttachments = useConversationStore((state) => state.stagedAttachments);
      ...
      >
        ...
        {attachments.length > 0 && (
          <div className="flex flex-wrap gap-2 px-6 pt-4">
            {attachments.map((file) => (
              <div key={file.id} className="flex items-center gap-2 bg-background/50 border border-border rounded-xl px-3 py-1.5 text-sm group relative">
                <FileIcon className="h-4 w-4 text-cyan-500" />
                <span className="max-w-[120px] truncate text-foreground/80">{file.filename}</span>
                {file.status === 'uploading' && <Loader2 className="h-3 w-3 animate-spin text-cyan-500" />}
        ...
        <div className="flex items-center justify-between px-2 pb-2">
          <Button 
            type="button" 
            variant="ghost" 
            className="rounded-full text-muted-foreground hover:text-foreground gap-2 ml-2 mb-1"
